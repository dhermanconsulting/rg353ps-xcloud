...
struct Game {
    ...
    // Also from fetch_names(), out of the same response we were already
    // fetching and discarding. The Store's Category is a single string like
    // "Racing & flying" or "Action & adventure"; Subcategory is null on every
    // title checked. Rating is out of 5 with the count behind it, so a 4.8
    // from 2.2 million can be told from a 4.4 from twenty.
    std::string genre;
    float rating = 0;        // 0 = unrated / not fetched
    int rating_count = 0;

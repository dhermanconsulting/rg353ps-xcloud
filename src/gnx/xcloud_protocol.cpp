...
std::vector<std::string> startup_messages(int width, int height,
    ...
    out.push_back(wrap_message(
        ...
        json{{"supportsCustomResolution", true},
             ...
             {"maxWidth", width},
             {"maxHeight", height},
             // maxPixels is named alongside maxWidth/maxHeight in the GDK's
             // XGameStreamingDisplayDetails as a bound titles are asked to
             // respect. We omitted it entirely until 2026-09-05; it is sent
             // now so that the "we never even told it" explanation is closed
             // off. See docs/RESOLUTION.md.
             {"maxPixels", (int64_t)width * height},

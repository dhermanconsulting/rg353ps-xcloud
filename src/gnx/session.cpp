...
const char* os_name(QualityTier tier) {
    switch (tier) {
        ...
        case QualityTier::P720HQ: return "android";
...
// os_override / display overrides are research hooks: the tier normally moves
// the fingerprint, the claimed display size and the resolution alias all at
// once, which makes it impossible to tell which of them the server is reading.
// Empty / zero means "use the tier's own value". See docs/RESOLUTION.md.
std::string device_info_header(QualityTier tier,
                               const std::string& os_override = "",
                               int display_w = 0, int display_h = 0) {
    json info = {
        {"appInfo",
         {{"env",
        ...
        {"dev",
         {{"hw", {{"make", "Microsoft"}, {"model", "unknown"}, {"sdktype", "web"}}},
          {"os",
           {{"name", os_override.empty() ? os_name(tier) : os_override.c_str()},
            {"ver", "22631.2715"},
            {"platform", "desktop"}}},
          {"displayInfo",
           ...
           {{"dimensions",
             {{"widthInPixels",
               display_w > 0 ? display_w
                             : (is_720_tier(tier) ? 1280 : 1920)},
              {"heightInPixels",
               display_h > 0 ? display_h
                             : (is_720_tier(tier) ? 720 : 1080)}}},
...
std::vector<std::string> GssvSession::headers() const {
    return {
        ...
        device_info_header(tier_, os_name_override_, display_w_override_,
                           display_h_override_),
        "Authorization: Bearer " + credentials_.token,
    };
}

void GssvSession::set_device_override(std::string os_name, int display_w,
                                      int display_h) {
    os_name_override_ = std::move(os_name);
    display_w_override_ = display_w;
    display_h_override_ = display_h;
}
...
int fetch_wait_time(Http& http, const EndpointCredentials& credentials,
                    const std::string& title_id, std::string* raw) {
    HttpResponse response;
    try {
        response = http.get(credentials.host + "/v1/waittime/" + title_id,
                            {"Authorization: Bearer " + credentials.token});
    } catch (const std::exception&) {
        return -1;  // Never worth failing a session over.
    }
    if (raw) *raw = response.body;
    if (!response.ok()) return -1;

    json value = json::parse(response.body, nullptr, false);
    if (!value.is_object()) return -1;

    // In the order a queue position is most usefully reported. Whichever of
    // these the backend actually sends, the first one present wins.
    static const char* const kFields[] = {
        "estimatedTotalWaitTimeInSeconds",
        "estimatedProvisioningWaitTimeInSeconds",
...
std::string sdp_set_video_caps(const std::string& sdp, int max_fs,
    ...
    // Unlike the two helpers above this does not preserve length: the OFFER
    // is ours to shape (we already lengthen it with ";stereo=1"), and only
    // the server's ANSWER must be passed through untouched.
    //
    // These are the receiver's declared H.264 decode limits. A spec-compliant
    // encoder may not exceed them, which makes this a far stronger statement
    // than any application-level request -- max-fs is a frame-size ceiling in
    // macroblocks, so 920 cannot be satisfied by a 1280x720 frame (3600).
    auto replace_value = [](std::string& text, const std::string& key,
    ...
    std::string out = sdp;
    if (max_fs > 0)
        replace_value(out, "max-fs=", std::to_string(max_fs));
    if (max_mbps > 0)
        replace_value(out, "max-mbps=", std::to_string(max_mbps));
    if (!level.empty())
        replace_value(out, "profile-level-id=", level);
    return out;
}

std::string sdp_set_h264_level_32(const std::string& sdp) {
    // Same-length in-place replacement, as above: the answer is matched
    // against this offer byte for byte, so nothing else may shift.
    const std::string needle = "profile-level-id=42e01f";
    std::string out = sdp;
    size_t at = out.find(needle);
    if (at != std::string::npos)
        out.replace(at + needle.size() - 6, 6, "42e020");
    return out;
}

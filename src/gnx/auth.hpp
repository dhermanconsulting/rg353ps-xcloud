...
public:
    ...
    // Round-trip the token store in <dir> and report whether it survived.
    //
    // This exists because save_refresh_token() is otherwise reachable only
    // from a live sign-in, so nothing offline covers the one path that
    // decides whether a user stays signed in -- and it is filesystem
    // behaviour, so it has to be checked where it will actually run
    // (`xcloud -selftest` on the handheld) rather than only on a build host.
    //
    // Uses its own filename and deletes it afterwards; it never reads,
    // writes or removes a real tokens.json. Detail of any failure, and of a
    // filesystem that cannot restrict permissions, goes into *detail.
    static bool selftest_store(const std::string& dir, std::string* detail);

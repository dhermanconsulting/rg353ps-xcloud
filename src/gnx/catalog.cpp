...
void fetch_names(Http& http, const std::vector<Game*>& games,
    ...
    for (size_t start = 0; start < ids.size(); start += kBatch) {
        ...
        for (const json& product : parsed.value("Products", json::array())) {
            ...
            // Genre, rating and year ride along in the same response; we
            // asked for fieldsTemplate=Details and were throwing them away.
            const json& props = product["Properties"];
            if (props.is_object() && props["Category"].is_string())
                found->second->genre = props["Category"].get<std::string>();

            const json markets = product.value("MarketProperties",
                                               json::array());
            if (markets.empty()) continue;
            const json& market_props = markets.front();
            // UsageData is a list per aggregation window (7Days, 30Days,
            // AllTime); the widest sample is the one worth showing.
            for (const json& usage :
                 market_props.value("UsageData", json::array())) {
                int count = usage.value("RatingCount", 0);
                if (count <= found->second->rating_count) continue;
                found->second->rating_count = count;
                found->second->rating = usage.value("AverageRating", 0.0);
            }
            const std::string released =
                market_props.value("OriginalReleaseDate", "");
            if (released.size() >= 4)
                found->second->year = std::atoi(released.substr(0, 4).c_str());
        }
    }
}

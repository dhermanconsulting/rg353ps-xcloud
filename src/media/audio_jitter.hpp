...
// packet and skipping a genuinely lost one.
//
// The hold is bounded by TIME, not by a packet count. green-vita's original
// rule (skip once 32 later packets have piled up) meant a single lost packet
// silenced output for 640 ms and then released 31 packets in one burst -- on
// this device that burst was the trigger for a self-sustaining underrun/shed
// oscillation (docs/KNOWN-ISSUES.md issue 2). Two packets' worth of wait
// covers any reordering a real network does; after that the gap is loss.
class AudioJitterBuffer {
public:
    // Add one arriving Opus payload tagged with its RTP sequence number, at
    // time now_ms (any monotonic millisecond clock).
    void push(uint16_t seq, const uint8_t* data, size_t size, uint64_t now_ms);
    ...
    bool pop(std::vector<uint8_t>& out, uint64_t now_ms);
    ...
    // Packets held back waiting for an earlier sequence number.

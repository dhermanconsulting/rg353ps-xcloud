...
//
// 120 ms rather than green-nx's 200: this is also the length of the freeze
// on every loss the retransmit does NOT repair, and on this handheld's Wi-Fi
// those are the common case (a five-minute race: 64 NACKs, 25 frames still
// dropped after the hold). The route to xCloud's UK edge is ~20-30 ms, so
// 120 ms still covers several retransmit round trips.
constexpr uint64_t kHoldMs = 120;
constexpr uint64_t kRenackMs = 70;
...
// Re-request every sequence number this incomplete frame lacks between its
// lowest packet and its marker (or, with the marker itself missing, two past
// its highest). One NACK per 17 seqs, like the first request.
void renack_missing(const VideoJitterBuffer::FrameView& f,
                    const VideoJitterBuffer::NackFn& nack) {
    if (f.seqs.empty()) return;
    uint16_t lo = f.seqs.front(), hi = f.seqs.front();
    bool have_marker = false;
    uint16_t marker = 0;
    for (size_t i = 0; i < f.seqs.size(); ++i) {
        uint16_t s = f.seqs[i];
        if (static_cast<int16_t>(s - lo) < 0) lo = s;
        if (static_cast<int16_t>(s - hi) > 0) hi = s;
        if (f.markers[i]) { have_marker = true; marker = s; }
    }
    uint16_t end = have_marker ? marker : static_cast<uint16_t>(hi + 2);
    std::vector<uint16_t> missing;
    for (uint16_t s = lo; s != static_cast<uint16_t>(end + 1); ++s) {
        bool have = false;
        for (uint16_t q : f.seqs)
            if (q == s) { have = true; break; }
        if (!have) missing.push_back(s);
        if (missing.size() > 64) break;  // a frame is never that big
    }
    for (size_t i = 0; i < missing.size();) {
        uint16_t pid = missing[i];
        uint16_t blp = 0;
        size_t j = i + 1;
        while (j < missing.size()) {
            uint16_t d = static_cast<uint16_t>(missing[j] - pid);
            if (d == 0 || d > 16) break;
...
void VideoJitterBuffer::receive(const uint8_t* rtp, size_t len, uint64_t now_ms,
    ...
    for (;;) {
        ...
        if (try_assemble(front, au, &marker_seq)) {
            ...
            if (waiting_keyframe_) {
                if (!has_keyframe(au)) {  // still no clean IDR -> keep dropping
                    /*
                     * Say WHAT is arriving, once a second, while we are
                     * blind. A stall here is indistinguishable from a dead
                     * link in the counters -- both are "frames=0, resync
                     * climbing" -- and the difference is entirely in which
                     * NAL types the server is actually sending. Rate limited
                     * because this runs per discarded frame, i.e. 60 a
                     * second, on the thread that also assembles.
                     */
                    if (++discards_since_log_ >= 60) {
                        discards_since_log_ = 0;
                        char types[96] = {0};
                        size_t at = 0;
                        for (size_t i = 0; i + 4 < au.size() && at < 80; ++i)
                            if (!au[i] && !au[i + 1] && !au[i + 2] &&
                                au[i + 3] == 1) {
                                at += (size_t)std::snprintf(
                                        types + at, sizeof(types) - at, "%u,",
                                        (unsigned)(au[i + 4] & 0x1f));
                                i += 4;
                            }
                        std::fprintf(stderr,
                                     "jitter: blind, %zu B au, nal types [%s] "
                                     "(waiting for an IDR)\n",
                                     au.size(), types);
                    }
                    *want_keyframe = true;
        ...
        // Not assemblable yet. The NACK for each gap went out the moment the
        // gap was seen, but a NACK or its retransmit can itself be lost and
        // there is no second chance built into that path: on the handheld's
        // Wi-Fi, 25 of 64 NACKed frames were still dropped after the hold.
        // So ask once more for whatever this frame is still missing, at
        // 70 ms -- past one retransmit round trip (~25-40 ms), early enough
        // for the answer to land before the hold expires.
        if (!front.renacked && now_ms - front.first_seen_ms > kRenackMs) {
            front.renacked = true;
            FrameView view;
            for (const auto& pk : front.packets) {
                view.seqs.push_back(pk.seq);
                view.markers.push_back(pk.marker);
            }
            renack_missing(view, nack);
            stats_.nacks++;
        }
        // If it has waited long enough, give up on it (a packet was lost and
        // no retransmit came) and resync from the next clean keyframe --
        // never feed the decoder a broken reference frame.
